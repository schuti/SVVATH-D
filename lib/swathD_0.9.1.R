            ...
            tabsetPanel(type = "tabs",
                    tabPanel("Volcano Plot", fluidPage(
                        column(3, 
                            wellPanel(
#-Conditional panel ------------------------------------------------------------------    
                        selectInput("tool_select", label = "Selected method", list("Volcano", "HM", "ML"), selected = "Volcano"),
                          conditionalPanel(
                            condition = "input.tool_select == 'Volcano' ",
#--------------------------------------------------------------------------------------
                      ...
                      column(3, 
                            ...
                            #fluidRow(box(width = 12, wellPanel(
                              ...
                              #column(3, 
                                  ...
                                  #  selectInput("dbs_select", label = "Databases", choices = selectedEnrichr_dbs),
                              #),
                              #column(2, 
      ...
      fluidRow(
               column(3, 
                      ...
                      selectInput("dataFormat", "Choose data format", 
                           choices = c("Peakview SWATH full report", "Processed dataset")), #"up_proteins (csv)", "up_peptides (csv)", "sp|up|acc_proteins (csv)", "gn_proteins (csv)", "gn_peptides (csv)", "sp|up|acc_peptides (csv)"), selected = "Peakview SWATH full report"), # add later:  "MRM (Sciex TripleTOF)", "iTRAQ-4plex", "iTRAQ-8plex", "SILAC",
...
#------------------------------------------------------------------------------
    ...
    #  dir.create( paste0(dest, "quality") )
     # setwd( paste0(dest, "quality") ) 
      ...
      plot_corrHM <- corrHM + 
        geom_text(aes(label = value), color = "black", size = 5) + #Var2, Var1, 
        theme(
...
#  observeEvent(input$pathwayPlot, {
  ...
  observeEvent(input$re_enrichAnal, {
    withProgress(message = "Submit data for gene enrichment analysis", value = 0.1, {
      ...
      output$plot15 <- renderPlot(
        ggplot(head(re_enrich_table, n = as.numeric(input$top_n)), aes(reorder(stringr::str_trunc(stringr::str_wrap(Term, 20), 40), geneRatio), geneRatio), na.rm = TRUE) + 
            geom_point(aes(size = geneInput, colour = Adjusted.P.value)) + #(stat="identity") + 
          #scale_colour_gradient(low = "red", high = "black", name="Adjusted p-value")+ 
          scale_colour_gradient(low = "red", high = "yellow", breaks = c(0.05, 5e-8), limits = c(0, 0.05)) +
      ...
      volEnrich <- re_enrich_table %>% dplyr::select("Term", "geneInput", "geneTotal","geneRatio", "Adjusted.P.value", "Genes")
      volEnrich$geneRatio <- round(volEnrich$geneRatio, 3)
  ...
  observeEvent(input$cluster_enrichAnal, {
    withProgress(message = "Submit gene cluster for gene enrichment analysis", value = 0.1, {
      ...
      hmEnrich <- re_enrich2_table %>% dplyr::select("Term", "geneInput", "geneTotal","geneRatio", "Adjusted.P.value", "Genes")
      hmEnrich$geneRatio <- round(hmEnrich$geneRatio, 3)
  ...
  observeEvent(input$ml_enrichAnal, {
    ...
    output$plot32 <- renderPlot(
      ggplot(head(re_enrich3_table, n = as.numeric(input$top_n3)), aes(reorder(stringr::str_trunc(stringr::str_wrap(Term, 20), 40), geneRatio), geneRatio), na.rm = TRUE) +
        geom_point(aes(size = geneInput, colour = Adjusted.P.value)) + #(stat="identity") + 
        #scale_colour_gradient(low = "red", high = "black", name="Adjusted p-value")+ 
        scale_colour_gradient(low = "red", high = "yellow", breaks = c(0.05, 5e-8), limits = c(0, 0.05)) +
        #labs(y = "Enrichr.Combined.Score") +
    ...
    mlEnrich <- re_enrich3_table %>% dplyr::select("Term", "geneInput", "geneTotal","geneRatio", "Adjusted.P.value", "Genes")
    mlEnrich$geneRatio <- round(mlEnrich$geneRatio, 3)
  ...
  #  source("lib/swathD_ml2net.R", local = TRUE)
    ...
    EnrichNET <- re_enrich4_table %>% dplyr::select("Term", "geneInput", "geneTotal","geneRatio", "Adjusted.P.value", "Genes")
    EnrichNET$geneRatio <- round(EnrichNET$geneRatio, 3)
